...
func (s *Silencer) Mutes(lset model.LabelSet) bool {
	...
	if markerVersion == s.silences.Version() {
		// No new silences added, just need to change which of the
		// old silences are still revelant.
		if len(ids) == 0 {
	...
	// TODO: Only recreate ids if different.
	ids = make([]string, len(sils))
	for i, s := range sils {
		ids[i] = s.Id
	}
	sort.Strings(ids) // For comparability.
	// TODO: Only SetSilenced if newVersion or ids changed.
